 ...
 * `features/session/SessionService` (the provider) — delete `features/chat`
 * and `features/session` still compiles; delete `features/session` and only
 * this one interface (plus its wiring in `app/AppModule`) needs a new home.
 */
